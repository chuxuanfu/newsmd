...
Do not commit generated news output, local logs, private notes, API keys, SSH keys, or personal memory files.

## Maintainer Push

Initial push command sequence:

```bash
echo "# newsmd" >> README.md
git init
git add README.md
git commit -m "first commit"
git branch -M main
git remote add origin git@github.com:chuxuanfu/newsmd.git
git push -u origin main
```

For this prepared project, commit all public project files instead of only `README.md`:

```bash
git init
git add README.md .gitignore requirements.txt scripts/
git commit -m "Initial newsmd pipeline"
git branch -M main
git remote add origin git@github.com:chuxuanfu/newsmd.git
git push -u origin main
```
